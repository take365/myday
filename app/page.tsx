...
type EventMeta = { tags: string[]; sourceUrl?: string };
...
const tagOrder = ["オンライン", "関東地方", "中部地方", "関西地方"];
const tagClass: Record<string, string> = { オンライン: "online", 関東地方: "kanto", 中部地方: "chubu", 関西地方: "kansai" };
const fireTerraceChannelUrl = "https://discord.com/channels/1272562436393799760/1443782111587860674";
...
export default function Home() {
  ...
  const [activeTags, setActiveTags] = useState<string[]>(tagOrder);
  ...
  const isVisible = (event: EventItem) => { const tags = metadataFor(event).tags; return tags.length === 0 || tags.some((tag) => activeTags.includes(tag)); };
  ...
  return <main className="shell">
    ...
    <header className="topbar"><div className="brand"><span className="brand-mark">◒</span><span>イベントカレンダー</span><span className="brand-pill">{discordGuild ?? "DISCORD MEMBER CALENDAR"}</span></div><div className="top-actions"><a className="ghost-button" href="/calendar-guide">カレンダー連携方法</a><a className="ghost-button" href="/api-guide">APIガイド</a><span className="sync-label">{discordUser}</span><a className="ghost-button" href="/api/auth/discord/logout">ログアウト</a></div></header>
    <section className="dashboard">
      <div className="calendar-card"><div className="calendar-head"><div><p className="eyebrow">CALENDAR</p><h2>{monthNames[cursor.getMonth()]} <span>{cursor.getFullYear()}</span></h2></div><div className="tag-filters calendar-filters" aria-label="イベントタグ">{tagOrder.map((tag) => <button key={tag} className={`tag-filter tag-${tagClass[tag]} ${activeTags.includes(tag) ? "active" : ""}`} aria-pressed={activeTags.includes(tag)} onClick={() => setActiveTags((current) => current.includes(tag) ? current.filter((item) => item !== tag) : [...current, tag])}><span className="tag-swatch" />{tag}</button>)}</div><div className="month-actions"><button onClick={() => setCursor(new Date(today.getFullYear(), today.getMonth(), 1))}>今日</button><button onClick={() => changeMonth(-1)}>‹</button><button onClick={() => changeMonth(1)}>›</button></div></div><div className="week-row">{weekNames.map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{days.map((day) => { const key = dateKey(day); const dayEvents = events.filter((item) => item.date === key && isVisible(item)); return <button className={`day-cell ${day.getMonth() !== cursor.getMonth() ? "muted" : ""} ${key === selectedDate ? "selected" : ""} ${key === todayKey ? "today" : ""}`} key={key} onClick={() => setSelectedDate(key)}><span className="day-number">{day.getDate()}</span>{dayEvents.slice(0, 3).map((item) => { const meta = metadataFor(item); return <span className={`event-dot ${item.category === "生活" ? "green" : item.category === "予定" ? "yellow" : ""}`} key={item.id} onClick={(event) => { event.stopPropagation(); setDetailEvent(item); }}><span className="event-tags">{meta.tags.map((tag) => <i className={`event-tag tag-${tagClass[tag]}`} title={tag} key={tag} />)}</span>{item.title}</span>; })}</button>; })}</div></div>
    ...
    {detailEvent && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setDetailEvent(null); }}><div className="modal detail-modal"><div className="modal-head"><div><p className="eyebrow">DETAIL</p><h2>{detailEvent.title}</h2></div><button className="close-button" onClick={() => setDetailEvent(null)}>×</button></div><div className="detail-meta">{detailEvent.category !== "仕事" && <span className={`category ${detailEvent.category === "生活" ? "life" : detailEvent.category === "予定" ? "plan" : "work"}`}>{detailEvent.category}</span>}<span>{formatDate(detailEvent.date)}</span><span>{detailEvent.startTime} — {detailEvent.endTime}</span></div><div className="detail-tags">{metadataFor(detailEvent).tags.map((tag) => <span className={`tag-badge tag-${tagClass[tag]}`} key={tag}><span className="tag-swatch" />{tag}</span>)}</div>{metadataFor(detailEvent).sourceUrl && <a className="source-link" href={metadataFor(detailEvent).sourceUrl} target="_blank" rel="noreferrer">DiscordのFIREテラスで投稿を開く ↗</a>}{detailEvent.notes && <div className="detail-notes">{detailEvent.notes}</div>}{detailEvent.attachments?.length ? <div className="detail-files"><p className="eyebrow">ATTACHMENTS</p>{detailEvent.attachments.map((file) => <div className="file-preview" key={file.id}>{file.contentType.startsWith("image/") ? <img src={`/api/files/${file.id}`} alt={file.filename} /> : <span className="file-icon">⌕</span>}<div><strong>{file.filename}</strong><small>{Math.round(file.size / 1024)}KB</small></div><a href={`/api/files/${file.id}`} target="_blank" rel="noreferrer">開く</a></div>)}</div> : <div className="no-files">添付ファイルはありません</div>}<div className="detail-actions"><button className="danger-button" onClick={() => void deleteEvent(detailEvent)}>削除</button><button className="primary-button" onClick={() => openEdit(detailEvent)}>編集する</button></div></div></div>}
